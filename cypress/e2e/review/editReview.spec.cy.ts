...
describe('Successfully write review of shelter', () => {
  ...
  it('login user, and write and edit review of shelter', () => {
    ...
    cy.get('[data-cy=review-modal] .star-container:nth-child(5)').click()
    cy.get('textarea[data-cy=reviewText]')
      .clear()
      .type(
    ...
    cy.get('[data-cy=reviews-grid]').find('[data-cy=review]').should('exist')
    cy.get('[data-cy=shelter-rating]').should('have.text', '4.0 (1 review)')
    cy.get('[data-cy=applicant-name]').should('have.text', 'Test User')
    cy.get('[data-cy=review-text]').should(
      'have.text',
      `The adoption process at this shelter was truly exceptional. The team was well-informed, the environment was spotless, and it's evident that they deeply care about the animals' welfare. If you're considering adopting a pet, this place is top-notch.`
